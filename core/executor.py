...
class MinicodeExecutor(MinicodeVisitor):
    ...
    def __init__(self, console_output, simulation_panel=None, polinomios_panel=None, music_panel=None):
        ...
        # Inicializar entorno musical
        self.musica = EntornoMusical(music_panel=music_panel)
    ...
    def visitComando_musical(self, ctx: MinicodeParser.Comando_musicalContext):
        # Obtener el nombre de la nota desde nota_nombre (puede ser ID o SI)
        nota_ctx = ctx.nota_nombre()
        if nota_ctx.ID():
            nota = nota_ctx.ID().getText()
        elif nota_ctx.SI():
            nota = nota_ctx.SI().getText()
        else:
            nota = "do"  # fallback
